...
#define TSUSPEND 4320// 3days=4320
...
{
      if (cval == true) {
		if(info->input_suspend_old != ENABLE_SCHARGING ||
				info->charge_enable_old != ENABLE_BATT_CHARGING)
		{
			...
			info->input_suspend_old = ENABLE_SCHARGING;
			info->charge_enable_old = ENABLE_BATT_CHARGING;
		}
	} else {
		if(info->input_suspend_old != DISABLE_SCHARGING ||
				info->charge_enable_old != DISABLE_BATT_CHARGING)
		{
			...
			info->input_suspend_old = DISABLE_SCHARGING;
			info->charge_enable_old = DISABLE_BATT_CHARGING;
		}
	}
}
...
void hq_enable_power_path(struct mtk_charger *info)
{
	...
	{
		info->cmd_discharging = true;
		charger_dev_enable(info->chg1_dev, false); // normal false
		charger_dev_do_event(info->chg1_dev,
			EVENT_DISCHARGE, 0);
		charger_dev_enable_powerpath(info->chg1_dev, true); // normal true
		info->input_suspend_old = ENABLE_SCHARGING;
		info->charge_enable_old = DISABLE_BATT_CHARGING;
...
void hq_update_charge_state(struct mtk_charger *info)
{
	...
	{
		if(info->chr_count > TSUSPEND )
		{
			info->en_batt_protect = ENABLE_BATT_PROTECT;
			rc = sum_get_prop_from_battery(info, POWER_SUPPLY_PROP_CAPACITY ,&pval);
			if(rc < 0)
			{
				pr_err("%s: Fail to get bms capacity  %d\n",__func__,rc);
			} else {
				info->capacity = pval.intval;
				if(info->capacity > HIGHT_CAPACITY)
				{
					hq_enable_charging(info, false);
				} else if(info->capacity == HIGHT_CAPACITY) {
					hq_enable_power_path(info);
				} else if(info->capacity < LOW_CAPACITY){
					hq_enable_charging(info, true);
				}
			}
		} else {
...
/*TabA7 Lite code for P210209-04223 fix HV_CHARGER_STATUS by wenyaqi at 20210222 start*/
...
#endif
/*TabA7 Lite code for P210209-04223 fix HV_CHARGER_STATUS by wenyaqi at 20210222 end*/
...
static void ss_charger_check_status(struct mtk_charger *info)
{
	...
	if (info->batt_store_mode) {
		chr_err("%s:store mode is working\n", __func__);
		basic_retail_app(info);
	}
	#endif

	/*TabA7 Lite code for OT8-229 modify sw_ovp function by wenyaqi at 20201218 start*/
	...
	if (input_suspend_old == info->input_suspend &&
		...
		ovp_disable_old == info->ovp_disable) {
		#endif
			chr_debug("%s:same as last one,skip set.\n", __func__);
			return;
		}
	...
	if (info->input_suspend == true || info->ovp_disable == true) {
	#endif
	/*TabA7 Lite code for OT8-229 modify sw_ovp function by wenyaqi at 20201218 end*/
		info->cmd_discharging = true;
		charger_dev_enable(info->chg1_dev, false);
		charger_dev_do_event(info->chg1_dev,
			EVENT_DISCHARGE, 0);
