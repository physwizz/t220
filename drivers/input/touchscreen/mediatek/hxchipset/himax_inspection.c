...
char *g_rslt_data;
uint32_t g_rslt_data_test_len = 0;
char *g_rslt_data_test;
bool file_w_flag;
...
static int himax_self_test_data_init(void)
{
	...
	if (g_rslt_data_test == NULL) {
		...
		if (g_rslt_data_test == NULL) {
			E("%s,%d: Memory allocation falied!\n",
				__func__, __LINE__);
			ret = HX_INSP_MEMALLCTFAIL;
			goto err_malloc_rslt_data_test;
		}
	}
	...
	memset(g_rslt_data_test, 0x00, 5*5000  * sizeof(char));
	...
	snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
			+ strlen(HX_RSLT_OUT_FILE)+21),
			"%s%s_%s.txt", HX_RSLT_OUT_PATH, HX_RSLT_OUT_FILE, get_date_time_str());
...
err_malloc_inspection_setting_val:
	kfree(g_rslt_data_test);
	g_rslt_data_test = NULL;
err_malloc_rslt_data_test:
	kfree(g_rslt_data);
	g_rslt_data = NULL;
...
void himax_ito_test_work(struct work_struct *work)
{
	...
	struct filename *vts_name = NULL;
	mm_segment_t fs = 0;
	I("write ito data start g_rslt_data_test_len = %d\n",g_rslt_data_test_len);
	...
	mutex_unlock(&private_ts->ito_lock);
...
/*TabA7 Lite code for OT8-1408 by liupengtao at 20210127 end*/
        ...
        hx_print_ic_id();
        if (file_w_flag) {
            memcpy(g_rslt_data_test,g_rslt_data,g_rslt_data_len);
                //vfs_write(raw_file, g_rslt_data, g_rslt_data_len, &pos);
            g_rslt_data_test_len += g_rslt_data_len;
        }
                ...
                if (i < HX_LP_WT_NOISE) {
                        if (g_test_item_flag[i] == 1) {
                                ...
                                if (file_w_flag &&
                                        ((rslt & HX_INSP_EGETRAW) == 0) &&
                                        ((rslt & HX_INSP_ESWITCHMODE) == 0)) {
                                        memcpy(g_rslt_data_test + g_rslt_data_test_len,g_rslt_data,g_rslt_data_len);
                                        //vfs_write(raw_file, g_rslt_data,
                                        //g_rslt_data_len, &pos);
                                        g_rslt_data_test_len += g_rslt_data_len;
                                }
...
END:
        ...
        mutex_unlock(&private_ts->ito_lock);
        queue_delayed_work(private_ts->hx_ito_wq, &private_ts->hx_ito_test_wq, msecs_to_jiffies(100));
        I("%s:OUT\n", __func__);
        return ret;
}
...
void himax_inspection_init(void)
{
    ...
    mutex_init(&private_ts->ito_lock);
}
