...
bool tcpc_typec_is_act_as_sink_role(struct tcpc_device *tcpc_dev)
{
	bool as_sink = true;
	uint8_t cc_sum;
...
static inline bool typec_handle_cc_changed_entry(struct tcpc_device *tcpc_dev)
{
	...
	if (tcpc_typec_is_act_as_sink_role(tcpc_dev))
...
/*TabA7 Lite code for P210311-04924 by wenyaqi at 20210319 start*/
...
#ifdef CONFIG_USB_NOTIFY_PROC_LOG
	cc_status = ((change_enum_cc_voltage_status(typec_get_cc1()) << 32) |
					change_enum_cc_voltage_status(typec_get_cc2()));
	store_usblog_notify(NOTIFY_CCSTATE, (void *)&cc_status, NULL);
#endif
	if (typec_get_cc1() == TYPEC_CC_VOLT_SNK_1_5 || typec_get_cc2() == TYPEC_CC_VOLT_SNK_1_5 ||
		typec_get_cc1() == TYPEC_CC_VOLT_SNK_3_0 || typec_get_cc2() == TYPEC_CC_VOLT_SNK_3_0)
		/*plug in hub, not nonstd*/
		hub_plugin_flag = true;
	else
		hub_plugin_flag = false;
	/*TabA7 Lite code for P210311-04924 by wenyaqi at 20210319 end*/
